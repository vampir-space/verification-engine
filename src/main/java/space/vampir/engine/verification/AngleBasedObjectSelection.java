...
public class AngleBasedObjectSelection extends ObjectSelection{
    public AngleBasedObjectSelection(Scope<ModelSeedFragment> scope) {
        super(scope);
    }
    public List<ObjectSelectionRecord> getObjects(
        ...
        for (var object : objects.entrySet()) {
            if (object.getValue().getType() == targetType) {
                ...
                else if (distance <= posError) {
                    final double angleDiff = calculateDiff(targetX,targetY,backPositionX,backPositionY,theta);
                ...
                // otherwise, if it is not too far, we can check if it is visible
                    ...
                    double angleDiff = calculateDiff(targetX,targetY,backPositionX,backPositionY,theta);
    ...
    protected double calculateDiff(double targetX, double targetY, double backPositionX, double backPositionY, double theta) {
        final double backToObjectY = targetY - backPositionY;
        final double backToObjectX = targetX - backPositionX;
